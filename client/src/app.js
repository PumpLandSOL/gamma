...
function drawChart() {
  ...
  const col = up ? '#00c805' : '#ff4d68';
  // grid
  c.strokeStyle = 'rgba(11,36,19,.08)'; c.lineWidth = 1; c.font = "12px 'VT323'"; c.textBaseline = 'middle';
  for (let g = 0; g <= 4; g++) {
    const yy = pad.t + (h - pad.t - pad.b) * g / 4; const val = hi - (hi - lo) * g / 4;
    c.beginPath(); c.moveTo(pad.l, yy); c.lineTo(w - pad.r, yy); c.stroke();
    c.fillStyle = '#7d947f'; c.textAlign = 'left'; c.fillText(px(val, CUR.dp).replace('$', ''), w - pad.r + 6, yy);
  }
  // area fill
  const grad = c.createLinearGradient(0, pad.t, 0, h - pad.b);
  grad.addColorStop(0, up ? 'rgba(0,200,5,.16)' : 'rgba(255,77,104,.16)'); grad.addColorStop(1, 'rgba(255,255,255,0)');
  ...
  c.fillStyle = '#ffffff'; c.font = "12px 'VT323'"; c.textAlign = 'center';
  ...
  if (A && A.positions) for (const p of A.positions.filter((x) => x.sym === sel)) {
    [['entry', p.entry, 'rgba(71,102,79,.6)'], ['liq', p.liq, 'rgba(255,77,104,.75)']].forEach(([lbl, v, cc]) => {
